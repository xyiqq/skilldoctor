...
const PATTERNS: PatternRule[] = [
  {
    ...
    pattern:
      /\b(ignore|disregard|forget)\b.{0,40}\b(previous|prior|above|system)\b.{0,20}\b(instructions?|prompts?|rules?)\b/i,
  },
  {
    id: "audit/prompt-injection",
    severity: "error",
  ...
  },
  {
    ...
    pattern:
      /(-----BEGIN (?:RSA |OPENSSH |EC |DSA )?PRIVATE KEY-----)|(AKIA[0-9A-Z]{16})|(sk-[A-Za-z0-9]{20,})|(gh[pousr]_[A-Za-z0-9_]{20,})|(xox[baprs]-[A-Za-z0-9-]{10,})|(api[_-]?key\s*[:=]\s*['\"][A-Za-z0-9_\-]{16,})/i,
  },
  {
    ...
    pattern:
      /\b(read|cat|type|open|load|source|print|dump|copy|scp|upload|exfiltrat\w*)\b[\s\S]{0,120}(~\/\.ssh\/|~\/\.aws\/credentials|~\/\.gnupg\/|\bid_rsa\b|\bid_ed25519\b|\.env(?:\.[A-Za-z0-9]+)?\b|cookies?\.sqlite|%APPDATA%)/i,
  },
  {
  ...
  },
  {
    ...
    pattern:
      /\b(discord(?:app)?\.com\/api\/webhooks|webhook\.site|ngrok\.(io|app)|requestbin|pipedream\.net|hooks\.slack\.com)\b/i,
  },
  {
  ...
  },
  {
    ...
    pattern:
      /\b(overwrite|rewrite|replace|edit|modify|update)\b.{0,40}\b(this )?(skill\.md|your own skill|the installed skill)\b/i,
  },
  {
...
export const auditRules: Rule[] = [
  {
    ...
    check: ({ skill }) => {
      ...
      if (!UNCONSTRAINED_TOOLS.test(joined) && !tokens.includes("*")) return;
      return [
        {
    ...
    check: ({ skill, files }) => {
      ...
      for (const file of files) {
        for (const item of PATTERNS) {
          if (item.scriptsOnly && !file.relativePath.startsWith("scripts/")) continue;
