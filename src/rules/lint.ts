...
const RELATIVE_REF_RE =
  /(?:(?:\[[^\]]*\]\((?!https?:|mailto:|#)([^)]+)\))|(?:(?<![\w~.\-/\\])(?:scripts|references|assets)\/[A-Za-z0-9._/*?<>{}$-]+))/g;
  ...
  },
  {
    ...
    check: ({ skill }) => {
      ...
      for (const match of skill.body.matchAll(RELATIVE_REF_RE)) {
        ...
        if (cleaned.startsWith("~") || /^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(cleaned)) continue;
        if (REF_PLACEHOLDER_RE.test(cleaned)) continue;
