...
export const AGENT_FIELDS: Record<AgentId, Set<string>> = {
  claude: new Set([
    ...CORE_FIELDS,
    "hooks",
    "context",
...
export function supportedAgents(frontmatter: Record<string, unknown>): Record<AgentId, "yes" | "warn" | "no"> {
  ...
  const result = {} as Record<AgentId, "yes" | "warn" | "no">;
...
export const compatRules: Rule[] = [
  {
    ...
    check: ({ skill }) => {
      ...
      for (const key of Object.keys(skill.frontmatter)) {
        ...
        if (!AGENT_FIELDS.claude.has(key)) continue;
        const supported = AGENTS.filter((agent) => AGENT_FIELDS[agent].has(key));
        if (supported.length === AGENTS.length) continue;
        findings.push({
  ...
  },
  {
    ...
    check: ({ skill }) => {
      if (skill.frontmatter.context !== "fork") return;
      return [
        {
